...
function loadAPISpecs(config, type, namespace) {
  ...
  if (_.isEmpty(this.specs)) {
    ...
    this.specs = yaml.safeLoad(fs.readFileSync(API_SPECS, 'utf8'));
  }

  spec = _.mapKeys(this.specs[API_PREFIX][type], function (value, key) {
...
function API(config) {
  ...
  _.extend(this.endpoints, loadAPISpecs(config, 'token'));
  // default secret backend
  _.extend(this.endpoints, loadAPISpecs(config, 'secret'));
}
...
API.prototype.mountEndpoints = function mountEndpoints(config, type, namespace) {
  ...
  _.extend(this.endpoints, loadAPISpecs(config, type, namespace));
};
